...
struct EasyNotchApp: App {
    ...
    var body: some Scene {
        MenuBarExtra("EasyNotch", systemImage: "sparkle", isInserted: $showMenuBarIcon) {
            Button("Settings") {
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    var viewModels: [String: EasyNotchViewModel] = [:] // UUID -> EasyNotchViewModel
    var window: NSWindow?
    let vm: EasyNotchViewModel = .init()
    ...
    private func enableSkyLightOnAllWindows() {
        if Defaults[.showOnAllDisplays] {
            windows.values.forEach { window in
                if let skyWindow = window as? EasyNotchSkyLightWindow {
        ...
        Task {
            ...
            await MainActor.run {
                if Defaults[.showOnAllDisplays] {
                    ...
                    if let skyWindow = self.window as? EasyNotchSkyLightWindow {
                        skyWindow.disableSkyLight()
                    }
                }
            }
    ...
    private func createEasyNotchWindow(for screen: NSScreen, with viewModel: EasyNotchViewModel) -> NSWindow {
        let rect = NSRect(x: 0, y: 0, width: windowSize.width, height: windowSize.height)
        let styleMask: NSWindow.StyleMask = [.borderless, .nonactivatingPanel, .utilityWindow, .hudWindow]

        let window = EasyNotchSkyLightWindow(contentRect: rect, styleMask: styleMask, backing: .buffered, defer: false)
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !Defaults[.showOnAllDisplays] {
            ...
            let window = createEasyNotchWindow(
    ...
    @objc func screenConfigurationDidChange() {
        ...
        // CGRect is not Hashable before macOS 15, so frames are compared as arrays.
        let screensChanged =
    ...
    @objc func adjustWindowPosition() {
        if Defaults[.showOnAllDisplays] {
            ...
            for screen in NSScreen.screens {
                ...
                if windows[uuid] == nil {
                    let viewModel = EasyNotchViewModel(screenUUID: uuid)
                    let window = createEasyNotchWindow(for: screen, with: viewModel)
            ...
            if window == nil {
                window = createEasyNotchWindow(for: selectedScreen, with: vm)
            }

            if let window = window {
                positionWindow(window, on: selectedScreen)
