//
//  EasyNotchViewCoordinator.swift
//  EasyNotch
//
...
@MainActor
class EasyNotchViewCoordinator: ObservableObject {
    static let shared = EasyNotchViewCoordinator()
        ...
        if preferredScreenUUID == nil, let legacyName = legacyPreferredScreenName {
            ...
            if let screen = NSScreen.screens.first(where: { $0.localizedName == legacyName }),
               let uuid = screen.displayUUID {
                ...
                Log.lifecycle.info("Migrated display preference from name '\(legacyName, privacy: .public)' to UUID '\(uuid, privacy: .public)'")
            } else {
                // Fallback to main screen if legacy screen not found
                preferredScreenUUID = NSScreen.main?.displayUUID
                Log.lifecycle.warning("Could not find display named '\(legacyName, privacy: .public)', falling back to main screen")
            }
