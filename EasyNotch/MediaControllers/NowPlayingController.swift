...
final class NowPlayingController: ObservableObject, MediaControllerProtocol {
    ...
    func toggleShuffle() async {
        // The adapter's diff stream pushes the authoritative resulting state.
    ...
    private func setupNowPlayingObserver() async {
        ...
        guard MediaRemoteAdapterValidator.isFrameworkTrusted(at: frameworkPath) else {
            assertionFailure("MediaRemoteAdapter.framework failed code signature validation")
            return
        }
    ...
    private func handleAdapterUpdate(_ update: NowPlayingUpdate) async {
        ...
        if let dateString = payload.timestamp,
           let date = Self.parseTimestamp(dateString) {
...
actor JSONLinesPipeHandler {
    ...
    func readJSONLines<T: Decodable>(as type: T.Type, onLine: @escaping (T) async -> Void) async {
        do {
            ...
            Log.adapter.error("Error processing JSON stream: \(error, privacy: .public)")
        }
    }
    ...
    func close() async {
        do {
            ...
            Log.adapter.error("Error closing pipe handler: \(error, privacy: .public)")
        }
    }
}
