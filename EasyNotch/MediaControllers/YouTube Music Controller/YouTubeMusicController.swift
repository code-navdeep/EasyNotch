...
final class YouTubeMusicController: MediaControllerProtocol, @unchecked Sendable {
    ...
    func setFavorite(_ favorite: Bool) async {
        do {
            ...
            Log.network.error("YouTubeMusicController failed to set favorite: \(error, privacy: .public)")
        }
    }
    ...
    func updatePlaybackInfo() async {
        ...
        do {
            ...
            Log.network.error("YouTubeMusicController failed to update playback info: \(error, privacy: .public)")
        }
    }
    ...
    private func initializeIfAppActive() async {
        ...
        do {
            ...
            Log.network.error("YouTubeMusicController failed to initialize: \(error, privacy: .public)")
    ...
    private func setupWebSocketIfPossible(token: String) async {
        guard let wsURL = WebSocketURLBuilder.buildURL(from: configuration.baseURL) else {
            Log.network.error("YouTubeMusicController failed to build WebSocket URL")
            return
        }
        ...
        do {
            ...
            Log.network.error("YouTubeMusicController WebSocket connection failed: \(error, privacy: .public)")
            await scheduleReconnect()
        }
    }
    ...
    private func sendCommand(
        ...
        do {
            ...
            Log.network.error("YouTubeMusicController command failed: \(error, privacy: .public)")
        }
    }
