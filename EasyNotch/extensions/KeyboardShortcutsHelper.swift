...
extension KeyboardShortcuts.Shortcut {
    ...
    func toKeyEquivalent() -> KeyEquivalent? {
        ...
        guard let ptr = TISGetInputSourceProperty(source, kTISPropertyUnicodeKeyLayoutData) else {
            Log.ui.error("Could not get keyboard layout data")
            return nil
        }
        ...
        guard osStatus == noErr else {
            Log.ui.error("Key translation failed — code: \(String(format: "0x%04X", carbonKeyCode), privacy: .public), status: \(osStatus, privacy: .public)")
            return nil
        }
