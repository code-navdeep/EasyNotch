...
extension Color {
    func ensureMinimumBrightness(factor: CGFloat) -> Color {
        ...
        // Calculate perceived brightness using the formula: (0.2126*R + 0.7152*G + 0.0722*B)
        let perceivedBrightness = (0.2126 * red + 0.7152 * green + 0.0722 * blue)

        // Already bright enough — never darken
        if perceivedBrightness >= factor {
            return self
        }

        // Pure black can't be scaled up (division by zero) — use a neutral gray
