...
class MusicManager: ObservableObject {
    ...
    @Published var animations: EasyNotchAnimations = .init()
    ...
    private func setActiveControllerBasedOnPreference() {
        ...
        Log.media.debug("Preferred media controller: \(preferredType.rawValue, privacy: .public)")
    ...
    private func updateFromPlaybackState(_ state: PlaybackState) {
        ...
        if state.isPlaying != self.isPlaying {
            Log.media.debug("Playback state changed: \(state.isPlaying ? "Playing" : "Paused", privacy: .public)")
    ...
    private var lyricsCacheOrder: [String] = []
    ...
    @MainActor
    private func cacheLyrics(title: String, artist: String, plain: String, synced: [(time: Double, text: String)]) {
        let key = lyricsCacheKey(title: title, artist: artist)
        if lyricsCache[key] == nil {
            lyricsCacheOrder.append(key)
            if lyricsCacheOrder.count > lyricsCacheLimit {
                let evicted = lyricsCacheOrder.removeFirst()
                lyricsCache.removeValue(forKey: evicted)
            }
        }
        lyricsCache[key] = CachedLyrics(plain: plain, synced: synced)
    }

    /// True while `title`/`artist` still describe the currently playing track and the
    /// surrounding fetch task has not been superseded by a newer one.
    @MainActor
    private func lyricsFetchIsCurrent(title: String, artist: String) -> Bool {
        !Task.isCancelled && title == self.songTitle && artist == self.artistName
    }
    ...
    private func fetchLyricsIfAvailable(bundleIdentifier: String?, title: String, artist: String) {
        ...
        if let cached = lyricsCache[lyricsCacheKey(title: title, artist: artist)] {
            DispatchQueue.main.async {
            ...
            lyricsFetchTask = Task { @MainActor in
                ...
                do {
                    ...
                    if let result = try await AppleScriptHelper.execute(script), let lyricsString = result.stringValue, !lyricsString.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                        guard self.lyricsFetchIsCurrent(title: title, artist: artist) else { return }
                        let trimmed = lyricsString.trimmingCharacters(in: .whitespacesAndNewlines)
                        self.currentLyrics = trimmed
                        self.isFetchingLyrics = false
                        self.syncedLyrics = []
                        self.cacheLyrics(title: title, artist: artist, plain: trimmed, synced: [])
                        return
                    }
                } catch {
    ...
    private func fetchLyricsFromWeb(title: String, artist: String) async {
        ...
        do {
            ...
            guard lyricsFetchIsCurrent(title: title, artist: artist) else { return }
            ...
            if let jsonArray = try JSONSerialization.jsonObject(with: data) as? [[String: Any]],
               let first = jsonArray.first {
                ...
                self.cacheLyrics(title: title, artist: artist, plain: resolved, synced: self.syncedLyrics)
            } else {
                self.currentLyrics = ""
                self.isFetchingLyrics = false
                self.syncedLyrics = []
                self.cacheLyrics(title: title, artist: artist, plain: "", synced: [])
            }
        } catch {
            guard lyricsFetchIsCurrent(title: title, artist: artist) else { return }
            self.currentLyrics = ""
    ...
    /// Index of the last synced line with time <= elapsed, or nil when no synced lyrics exist.
    ...
    func lyricLine(at elapsed: Double) -> String {
        guard let idx = lyricLineIndex(at: elapsed) else { return currentLyrics }
    ...
    func openMusicApp() {
        guard let bundleID = bundleIdentifier else {
            Log.media.error("Cannot open music app: bundle identifier is nil")
            return
        }
        ...
        if let appURL = workspace.urlForApplication(withBundleIdentifier: bundleID) {
            ...
            workspace.openApplication(at: appURL, configuration: configuration) { (app, error) in
                if let error = error {
                    Log.media.error("Failed to launch app with bundle ID \(bundleID, privacy: .public): \(error, privacy: .public)")
                } else {
                    Log.media.info("Launched app with bundle ID \(bundleID, privacy: .public)")
                }
            }
        } else {
            Log.media.error("Failed to find app with bundle ID \(bundleID, privacy: .public)")
        }
    }
